...
export function runSensitivityAnalysis1D(
  ...
  const detailedResults: NonNullable<
    SensitivityAnalysis1D["results"]
  >["detailedResults"] = [];
  ...
  parameters.forEach((param) => {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      const modifiedInputs = setValueByPath(
        baseInputs,
        param.parameter,
        paramValue,
      );
...
export function runSensitivityAnalysis2D(
  ...
  // Le nombre total de cellules est déjà borné par maxSteps=50 ci-dessus
  ...
  for (let i = 0; i <= stepsX; i++) {
    xValues.push(
      parameterX.min + ((parameterX.max - parameterX.min) * i) / stepsX,
    );
  }
  ...
  for (let i = 0; i <= stepsY; i++) {
    yValues.push(
      parameterY.min + ((parameterY.max - parameterY.min) * i) / stepsY,
    );
  }
  ...
  for (let j = 0; j <= stepsY; j++) {
    const row: number[] = [];
    for (let i = 0; i <= stepsX; i++) {
      let modifiedInputs = setValueByPath(
        baseInputs,
        parameterX.parameter,
        xValues[i],
      );
      modifiedInputs = setValueByPath(
        modifiedInputs,
