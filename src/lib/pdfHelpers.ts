...
// ============================================================================
//
// NOTE: PDFContext est un état mutable par conception. L'API jsPDF est
// intrinsèquement impérative (setFont, text, addPage…). Toutes les fonctions
// ci-dessous mutent ctx.yPosition et ctx.currentPage en place.
// Ceci est une exception documentée à la règle d'immutabilité du projet.
...
function addPageHeader(ctx: PDFContext): void {
...
function addPageFooter(ctx: PDFContext): void {
  const oldFontSize = ctx.pdf.getFontSize();
  const oldTextColor = ctx.pdf.getTextColor();
...
function addSubsubsection(ctx: PDFContext, text: string): PDFContext {
...
export function addMetricGrid(
  ...
  metrics: Array<{
    label: string;
    value: string;
    color?: keyof typeof PDF_CONFIG.colors;
  }>,
): PDFContext {
  const cols = 2;
...
function addFormula(ctx: PDFContext, formula: string): PDFContext {
