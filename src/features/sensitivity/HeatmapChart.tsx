...
export function HeatmapChart({
  ...
  // --- Tous les hooks sont déclarés ci-dessus --- early returns safe à partir d'ici ---
  ...
  const isNearZero = (val: number) =>
    Math.abs(val) < maxAbsValue * ZERO_THRESHOLD;
  ...
  const getColor = (value: number) => {
    ...
    if (value < 0) {
      ...
      const g = Math.floor(
        COLOR_RED_LIGHT.g - intensity * (COLOR_RED_LIGHT.g - COLOR_RED_DARK.g),
      );
      const b = Math.floor(
        COLOR_RED_LIGHT.b - intensity * (COLOR_RED_LIGHT.b - COLOR_RED_DARK.b),
      );
      ...
      const r = Math.floor(
        COLOR_GREEN_LIGHT.r -
          intensity * (COLOR_GREEN_LIGHT.r - COLOR_GREEN_DARK.r),
      );
      const g = Math.floor(
        COLOR_GREEN_LIGHT.g -
          intensity * (COLOR_GREEN_LIGHT.g - COLOR_GREEN_DARK.g),
      );
      const b = Math.floor(
        COLOR_GREEN_LIGHT.b -
          intensity * (COLOR_GREEN_LIGHT.b - COLOR_GREEN_DARK.b),
      );
      return `rgba(${r}, ${g}, ${b}, 0.9)`;
    }
  };
  ...
  const formatParamValue = (value: number, paramPath: string) => {
    if (
      paramPath.includes("occupancyRate") ||
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Échec silencieux de l'export image — l'UX n'est pas interrompue
    } finally {
            ...
            <thead>
              <tr>
                ...
                {xValues.map((x, i) => (
                  <th
                    key={i}
                    className="border border-slate-300 bg-slate-100 px-1 py-0.5 text-[10px] min-w-[60px]"
                  >
  ...
  return (
    <>
      ...
      <div className="space-y-4">
        ...
        {/* Contenu du heatmap */}
        <div ref={heatmapRef}>{renderHeatmapContent()}</div>
      </div>
